 ...
 * Mappings specific for Oracle
 * @version $Revision 1.0$
 * @since 1.0
 */
public class OracleStrategy implements MappingStrategy {
     ...
     * @param delegate
     */
    public OracleStrategy(MappingStrategy delegate) {
    ...
    public void setSQLParam(final PreparedStatement stmt, final int idx, SQLParam parm) throws SQLException {
        switch (parm.getSqlType()) {
        case Types.BIT:
        case Types.BOOLEAN:
            // oracle does not know boolean and uses char(1)
            // setNull will throw an exception if BOOLEAN is used as a type...            
