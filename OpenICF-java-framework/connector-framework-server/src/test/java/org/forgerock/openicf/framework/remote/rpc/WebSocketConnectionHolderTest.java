 ...
 * run concurrently, and a failing task does not stall the queue.
 */
        ...
        for (int i = 0; i < taskCount; i++) {
            ...
            holder.executeSerially(pool, new Runnable() {
                public void run() {
                    ...
                    // executed is not synchronized on purpose: the serial
                    // queue must never run two tasks at the same time.
                    executed.add(id);
