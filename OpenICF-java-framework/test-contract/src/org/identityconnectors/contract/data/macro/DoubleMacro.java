...
/**
 * {@link Macro} implementation which resolves value as {@link Double}
 * 
 * @author Dan Vernon
 */
public class DoubleMacro implements Macro {

    private static final Log LOG = Log.getLog(DoubleMacro.class);
    ...
    public Object resolve(Object[] parameters) {
        ...
        String doubleString = (String)parameters[1];

        // trim and get
        doubleString = doubleString.trim();
        Double value = new Double(doubleString);
        
        LOG.ok("''{0}'' macro with parameter ''{1}'' resolves to (''{2}'',''{3}'')", getName(), doubleString, value.getClass().getName(), value.toString());        
        return value;
    }
