...
/**
 * {@link Macro} implementation which resolves {@link Float}
 * 
 * @author Dan Vernon
 */
public class FloatMacro implements Macro {

    private static final Log LOG = Log.getLog(FloatMacro.class);
    ...
    public Object resolve(Object[] parameters) {
        ...
        String floatString = (String)parameters[1];

        // trim and get
        floatString = floatString.trim();
        Float value = new Float(floatString);
        
        LOG.ok("''{0}'' macro with parameter ''{1}'' resolves to (''{2}'',''{3}'')", getName(), floatString, value.getClass().getName(), value.toString());        
        return value;
    }
