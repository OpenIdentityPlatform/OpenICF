 ...
 * <code>Type</code> value specifies how the method must apply to the target
 * object the values of the attributes that the input ConnectorObject contains.
 * <p>
 * This <em>incremental</em> update requires a more sophisticated
 * implementation than <code>UpdateOp</code>, which replaces the
 * corresponding attributes of the target object with the attributes (and
 * attribute values) from the input ConnectorObject but does not take a
 * <code>Type</code> argument. (In effect, <code>UpdateOp</code> acts as if
 * it received a value of {@link Type#REPLACE Type.REPLACE}.)
 * <p>
 * The developer of a Connector should implement only one of
 * <code>UpdateOp</code> or <code>AdvanceUpdateOp</code>; there is no need
 * to implement both. The common code in the framework prefers
 * <code>AdvanceUpdateOp</code> if <code>AdvanceUpdateOp</code> is
 * implemented. If <code>AdvanceUpdateOp</code> is not implemented, then the
 * common code in the framework uses <code>UpdateOp</code> and performs the
 * additional processing that is needed to support incremental update. The
 * common code fetches the current <code>ConnectorObject</code>, applies the
 * values from the attributes of the input <code>ConnectorObject</code>, and
 * passes the resulting <code>ConnectorObject</code> as input to the
 * Connector's implementation of <code>UpdateOp</code>.
 * 
 * @author Will Droste
 * @version $Revision $
 * @since 1.0
...
public interface AdvancedUpdateOp extends SPIOperation {
    ...
    enum Type {
         ...
         * input <code>ConnectorObject</code> contains, then add this
         ...
         * <p>
         * For each attribute that the input ConnectorObject contains, add to
         * the current values of that attribute in the target object all of the
         * values of that attribute in the input ConnectorObject.
         * <p>
         * NOTE that this does not specify how to handle duplicate values. The
         * general assumption for an attribute of a ConnectorObject is that the
         * values for an attribute may contain duplicates. Therefore, in general
         * simply <em>append</em> the provided values to the current values
         * for each attribute.
         */
        ADD,
        /**
         ...
         * <p>
         * For each attribute that the input ConnectorObject contains, remove
         * from the current values of that attribute in the target object any
         * value that matches one of the values of that attribute in the input
         * ConnectorObject.
         * <p>
         * NOTE that this does not specify how to handle unmatched values. The
         * general assumption for an attribute of a ConnectorObject is that the
         * values for an attribute are merely <i>representational state</i>.
    ...
    /**
     * Modify the target object based on the specified <code>type</code> and
     * the information provided in the input <code>ConnectorObject</code>.
     * <p>
     ...
     *            values that the input ConnectorObject contains. For details,
     *            see {@link Type}.
     * 
     * @param objclass
     *            class of object to modify.
     * @param attrs
     *            set of all attributes with delta, plus the {@link Uid} of the
     *            object. Only the values deltas are passed in the {@link Attribute}.
     * @param options
     ...
     * 
     * @return the <code>Uid</code> of the updated object in case the update
