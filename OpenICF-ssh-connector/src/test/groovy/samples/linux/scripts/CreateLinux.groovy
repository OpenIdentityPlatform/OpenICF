...
def ALREADY_EXIST = "already exists"
...
def sudo = "sudo -k "
def useradd = "/usr/sbin/useradd "
def groupadd = "/usr/sbin/groupadd "
def command = ""
...
switch (objectClass.getObjectClassValue()) {
    case ObjectClass.ACCOUNT_NAME:
        ...
        def description = createAttributes.findString("description") ? "-c '" + createAttributes.findString("description") + "' " : ""
        def home = createAttributes.findString("home") ? "-d " + createAttributes.findString("home") + " " : ""
        def group = createAttributes.findString("group") ? "-g " + createAttributes.findString("group") + " " : ""
        def shell = createAttributes.findString("shell") ? "-s " + createAttributes.findString("shell") + " " : ""
        def uid = createAttributes.findString("uid") ? "-u " + createAttributes.findString("uid") + " " : ""
        command = sudo + useradd + description + home + group + shell + uid + id
        log.info("Command is: $command")
        break
    case ObjectClass.GROUP_NAME:
        // /usr/sbin/groupadd <groupname>
        command = sudo + groupadd + id
        break
}
...
expect prompt, {
    ...
    if (message.endsWith(ALREADY_EXIST)) {
